...
def process_frame_json():
    ...
    for element in frame_information_json["matrix"]:
        if element["time"] < pre_t:
            assert half == "first", f"Error: {element['time']} < {pre_t} in {half} half"
            half = "second"
        pre_t = element["time"]
...
def visulize_one_frame(ht, half, frame_info):
    info = None
    ...
    get_pitch(positions_dict, directions_dict)
    cap.release()
...
if __name__ == "__main__":
    ...
    #     all_time_half.append((frame["htime"], frame["half"]))
    #     print(frame["htime"], frame["half"])
    ...
    visulize_one_frame("12:28", "first", new_frame)
